...
| TIM1_CH1 (PWM) | PA8 | 復用推挽輸出，10 kHz PWM，分辨率 1200 |
...
|---|---|---|---|---|---|
| CH1 | PA8 | TIM1 (高階) | APB2 = 72 MHz | 10 kHz | 1200 |
...
```
定時器時脈 = 72 MHz
PSC       = 5     → 計數器時脈 = 72 MHz / (5+1) = 12 MHz
ARR       = 1199  → PWM 頻率    = 12 MHz / (1199+1) = 10 kHz
分辨率    = ARR+1 = 1200 階（0–1200 對應 0%–100% 佔空比）
```

> TIM4_CH1（PB6）保持 1 kHz（PSC=71, ARR=999），因其 TRGO 用於觸發 ADC1 注入組。
...
```c
App_Pwm1_SetDuty(600);   // PA8 輸出 50% 佔空比 (0–1200)
App_Pwm4_SetDuty(250);   // PB6 輸出 25% 佔空比 (0–1000)
```

佔空比參數範圍：**TIM1 為 0–1200**，**TIM4 為 0–1000**，線性對應 0.0%–100.0%。
